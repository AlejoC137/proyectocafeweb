...
const ImportExportPanel = ({ model, activeCustomization, onImportConfig }) => {
  ...
  const generateJSXCode = () => {
    const colors = activeCustomization?.selected_colors || { base: '#d97706', secondary: '#451a03', accent: '#f59e0b' };
...
export default function Interactive3DModel() {
  const modelConfig = {
    title: "${model?.title || 'Modelo Café 3D'}",
    fileUrl: "${model?.file_url || 'https://proyectocafeweb.vercel.app/models/cafe.glb'}",
  ...
  const generateJSONConfig = () => {
    const config = {
      model_id: model?.id || 'cafe-3d-model',
      title: model?.title || 'Modelo 3D Café',
...
export default model3dSlice.reducer;`;
  ...
  const handleDownloadJSON = () => {
    ...
    a.href = url;
    a.download = `modelo_3d_${model?.id || 'cafe'}_config.json`;
    a.click();
  ...
  const handleImportSubmit = (e) => {
    ...
    try {
      ...
      setImportText('');
      alert('¡Configuración 3D importada con éxito!');
    } catch (err) {
      setImportError('El formato introducido no es un JSON válido.');
    }
  };
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-white shadow-xl flex flex-col gap-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-xl text-xs transition-all shadow-md flex items-center gap-1.5"
          >
            💾 Descargar .JSON
          </button>
        </div>
      ...
      <div className="relative">
        ...
        <button
          type="button"
          onClick={handleCopy}
          className="absolute top-3 right-3 px-3 py-1 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-medium border border-slate-700 shadow-sm transition-all"
        >
      ...
      <form onSubmit={handleImportSubmit} className="pt-2 border-t border-slate-800 flex flex-col gap-3">
        ...
        <textarea
          ...
          placeholder="Pega aquí un JSON de configuración 3D para aplicar colores y estilos de inmediato..."
        ...
        <div className="flex justify-end">
          <button
            ...
            className="px-4 py-2 bg-slate-800 hover:bg-amber-500 hover:text-slate-950 disabled:opacity-50 text-slate-200 rounded-xl text-xs font-bold transition-all border border-slate-700"
          >
