...
function AccionesRapidasMenuLunch() {
  const dispatch = useDispatch();
  ...
  const initialMenuItemData = {
    ...
    SUB_GRUPO: TARDEO_ALMUERZO,
    Precio: "",
    Comp_Lunch: null, // Mejor inicializar como null
    Foto: "",
    Estado: "Activo",
    PRINT: false,
    Order: "",
  ...
  const handleCrearMenuItem = async () => {
    ...
    try {
      ...
      // Limpia campos vacíos y asegura que el precio sea un número
      Object.keys(menuItemToSubmit).forEach(key => {
        if (menuItemToSubmit[key] === "" || menuItemToSubmit[key] === null) {
  ...
  // Función para obtener los datos iniciales para el formulario hijo
  const getInitialLunchData = () => {
    try {
      if (menuItemData.Comp_Lunch && typeof menuItemData.Comp_Lunch === 'string') {
        return JSON.parse(menuItemData.Comp_Lunch);
      }
    } catch (e) {
      console.error("Error al parsear Comp_Lunch para el formulario hijo:", e);
    }
    return null;
  };
