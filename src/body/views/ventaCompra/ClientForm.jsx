...
export default function ClientForm({ onClose, initialData = "" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Desactivado temporalmente para no usar la tabla UserMessages
      // try {
      //   await supabase
      //     .from("UserMessages")
      //     .insert({
  ...
  const getWhatsAppUrl = (phone, text) => {
    // Solo enviamos un saludo básico por la URL para evitar errores de encoding
    const encoded = encodeURIComponent("¡Hola!");
  ...
  const handleWhatsAppWelcome = () => {
    ...
    // Copiamos el mensaje completo al portapapeles
    navigator.clipboard.writeText(content).catch(err => {
      console.error('Error al copiar al portapapeles: ', err);
    });

    const whatsappUrl = getWhatsAppUrl(phone, content);
