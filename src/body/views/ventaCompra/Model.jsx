...
// --- Componentes de Filas (SIN CAMBIOS) ---
const EmployeeRow = ({ employee, onUpdate, onRemove }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-center mb-3 p-2 bg-gray-50 rounded-md">
            ...
            <div className="flex items-center"><span className="text-gray-500 mr-2">$</span><input type="number" placeholder="Valor/Hora" value={employee.hourlyRate} onChange={(e) => onUpdate({ ...employee, hourlyRate: parseFloat(e.target.value) || '' })} className="p-2 border rounded-md shadow-sm w-full"/></div>
...
const FixedCostRow = ({ cost, onUpdate, onRemove }) => {
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center mb-3 p-2 bg-gray-50 rounded-md">
            ...
            <div className="flex items-center"><span className="text-gray-500 mr-2">$</span><input type="number" placeholder="Valor Mensual" value={cost.value} onChange={(e) => onUpdate({ ...cost, value: parseFloat(e.target.value) || '' })} className="p-2 border rounded-md shadow-sm w-full"/></div>
...
function Model() {
    ...
    useEffect(() => {
        if (activeModel) {
            ...
            const hasOldFixedCosts = ['serviciosPublicos', 'arriendo', 'impuestos', 'serviciosExternos', 'mantenimiento'].some(key => modelCopy.costs[key] !== undefined);
            if (!modelCopy.costs.fijos && hasOldFixedCosts) {
                modelCopy.costs.fijos = [];
                const oldFixedMap = { 'Servicios Públicos': 'serviciosPublicos', 'Arriendo': 'arriendo', 'Impuestos': 'impuestos', 'Servicios Externos': 'serviciosExternos', 'Mantenimiento': 'mantenimiento' };
            ...
            // Carga los datos de la simulación guardada desde 'esquema'
            ...
            if (savedSimulation && Array.isArray(savedSimulation)) {
                ...
                // Si no hay datos, resetea el estado de la simulación
    ...
    const calculateRequireSells = (costs) => { if (!costs) return 0; const personal = (costs.personal || []).reduce((acc, emp) => acc + (emp.weeklyHours || 0) * (emp.hourlyRate || 0) * 4.33, 0); const otros = (costs.otros || []).reduce((acc, cost) => acc + (cost.value || 0), 0); const fijos = (costs.fijos || []).reduce((acc, cost) => acc + (cost.value || 0), 0); const total = personal + otros + fijos; const margin = (costs.desiredProfitMargin || 0) / 100; return margin >= 1 ? total : total / (1 - margin); };
    ...
    const handleSaveChanges = () => {
        const newRequireSells = Math.round(calculateRequireSells(modifiedModel.costs));
    
        const simulacionVentasData = simulatedProductIds.map(id => ({
            productId: id,
            quantity: salesMix[id] || 0,
        }));
    
        const updatedData = {
            costs: modifiedModel.costs,
            requireSells: newRequireSells,
            esquema: {
                simulacionVentas: simulacionVentasData
            }
        };
        dispatch(updateModelAction(modifiedModel._id, updatedData));
    };

    const addNewModel = async () => { if (!newModelName.trim()) { alert("Por favor, dale un nombre al nuevo modelo."); return; } const newModelData = { name: newModelName, costs: { personal: [], fijos: [{ id: Date.now() + 1, name: 'Servicios Públicos', value: 0 },{ id: Date.now() + 2, name: 'Arriendo', value: 0 },{ id: Date.now() + 3, name: 'Impuestos', value: 0 },{ id: Date.now() + 4, name: 'Servicios Externos', value: 0 },{ id: Date.now() + 5, name: 'Mantenimiento', value: 0 },], otros: [], desiredProfitMargin: 20, }, requireSells: 0, esquema: {} }; const createdModel = await dispatch(createModelAction(newModelData)); if (createdModel) { setActiveModel_id(createdModel._id); setNewModelName(""); } };
    ...
    const { totalPersonal, totalOtros, totalCostosFijos, grandTotal, utilidad, hasChanges, requireSells } = useMemo(() => {
        if (!modifiedModel || !activeModel) return { totalPersonal: 0, totalOtros: 0, totalCostosFijos: 0, grandTotal: 0, utilidad: 0, hasChanges: false, requireSells: 0 };
        ...
        const total = personal + otros + fijos;
        const reqSells = calculateRequireSells(costs);
        const utilidadCalculada = reqSells - total;
        
        // --- INICIO DE LA CORRECCIÓN ---
        // Compara si los costos han cambiado
        const costsHaveChanged = JSON.stringify(activeModel?.costs) !== JSON.stringify(modifiedModel.costs);

        // Compara si la simulación ha cambiado
        const originalSimulation = activeModel?.esquema?.simulacionVentas || [];
        const currentSimulation = simulatedProductIds.map(id => ({
            productId: id,
            quantity: salesMix[id] || 0
        }));
        const simulationHasChanged = JSON.stringify(originalSimulation) !== JSON.stringify(currentSimulation);

        // Activa si cualquiera de los dos ha cambiado
        const changesDetected = costsHaveChanged || simulationHasChanged;
        // --- FIN DE LA CORRECCIÓN ---

        return { totalPersonal: personal, totalOtros: otros, totalCostosFijos: fijos, grandTotal: total, utilidad: utilidadCalculada, hasChanges: changesDetected, requireSells: reqSells };
    }, [modifiedModel, activeModel, simulatedProductIds, salesMix]);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            {searchResults.length > 0 && (
                                <div className="absolute z-10 w-full bg-white border rounded-md mt-1 max-h-48 overflow-y-auto shadow-lg">
                                    {searchResults.map(product => (
                                        <div
                                            key={product._id}
                                            onClick={() => addProductToSimulation(product._id)}
                            ...
                            {simulatedProductIds.length > 0 ? simulatedProductIds.map(id => {
                                ...
                                return (
                                     <SimulationRow
                                        key={id}
                                        product={product}
                                        quantity={salesMix[id]}
                                        onQuantityChange={handleSalesMixChange}
                                        onRemove={removeProductFromSimulation}
                                    />
                                );
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white p-6 rounded-lg shadow-lg sticky top-8">
                        <h3 className="text-xl font-bold mb-4 text-gray-800 border-b pb-2"> Resumen del Modelo: <span className="text-blue-600">{modifiedModel.name}</span> </h3>
                        <div className="space-y-3 mt-4">
                            <div className="flex justify-between"><span>Total Personal:</span><span className="font-medium">{totalPersonal.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span></div>
                            <div className="flex justify-between"><span>Total Costos Fijos:</span><span className="font-medium">{totalCostosFijos.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span></div>
                            <div className="flex justify-between"><span>Total Otros Costos:</span><span className="font-medium">{totalOtros.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span></div>
                            <hr className="my-3"/>
                            <div className="flex justify-between items-center text-lg font-bold"><span>Costo Operativo Total:</span><span className="text-blue-700">{grandTotal.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span></div>
                        </div>
                        ...
                        <div className="mt-6 pt-4 border-t">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="flex items-center"><input type="number" value={modifiedModel.costs.desiredProfitMargin || ''} onChange={(e) => handleProfitMarginChange(e.target.value)} className="p-2 border rounded-md shadow-sm w-full text-right" min="0" max="99"/><span className="ml-2 text-gray-500">%</span></div>
                                </div>
                                <div className="p-3 bg-gray-50 rounded-md">
                                    <div className="flex justify-between items-center text-sm"><span>Punto de Equilibrio:</span><span className="font-bold">{grandTotal.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span></div>
                                </div>
                                ...
                                <div className="mt-4 pt-4 border-t">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="mt-2">
                                            <label className="text-sm text-gray-600">Progreso hacia la Meta de Ventas ({requireSells.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 })})</label>
