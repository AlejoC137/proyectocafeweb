...
import { User, Briefcase, Phone, Clock, ChevronDown, ChevronRight } from 'lucide-react';
...
const StaffTable = ({ employees }) => {
    ...
    const inactiveEmployees = employees.filter(emp => emp.Contratacion === false || emp.Contratacion === "false");
        ...
        empList.map((emp) => (
            ...
            >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${emp.Contratacion !== false && emp.Contratacion !== "false" ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                        {emp.Contratacion !== false && emp.Contratacion !== "false" ? 'Activo' : 'Inactivo'}
                    </span>
                </td>
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {/* Activos */}
                        {renderRows(activeEmployees)}

                        {/* Separador e Inactivos */}
                        {inactiveEmployees.length > 0 && (
                            <>
                                <tr>
                                    <td colSpan="7" className="bg-gray-50 p-0 border-y border-gray-200">
                                        <button
                                        ...
                                        >
                                            <div className="flex items-center">
                                                <span className="bg-gray-200 text-gray-700 py-0.5 px-2.5 rounded-full text-xs font-bold mr-3">
                                                    {inactiveEmployees.length}
                                                </span>
                                                Empleados Inactivos
                                            </div>
                                            {showInactive ? <ChevronDown className="w-5 h-5 text-gray-400" /> : <ChevronRight className="w-5 h-5 text-gray-400" />}
                                        </button>
                                    </td>
                                </tr>
                                {showInactive && renderRows(inactiveEmployees)}
                            </>
                        )}
