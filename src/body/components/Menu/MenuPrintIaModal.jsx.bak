...
CRÍTICO:
- La respuesta debe ser ÚNICAMENTE el objeto JSON válido. Sin markdown, sin explicaciones.
- Los IDs de bloques deben ser exactamente los nombres de las categorías o bloques especiales arriba indicados.`;
  };
  ...
  const handleVerifyJson = (jsonOverride = null) => {
    try {
      const rawToVerify = typeof jsonOverride === 'string' ? jsonOverride : jsonInput;
      if (!rawToVerify.trim()) {
        setVerificationResult({ valid: false, message: "Por favor pega el código JSON de la IA." });
        return;
      }

      let cleanText = rawToVerify.trim();
      ...
      if (!parsed.pages || !Array.isArray(parsed.pages) || parsed.pages.length === 0) {
        throw new Error("El JSON debe contener un array 'pages' con al menos una página.");
      }
      ...
      if (isHoriz) {
        pages.forEach((pg, pIdx) => {
          if (!pg.columns || !Array.isArray(pg.columns)) {
            throw new Error(`La página #${pIdx + 1} debe contener un array 'columns'.`);
          }
          totalColumns += pg.columns.length;
          pg.columns.forEach((col, cIdx) => {
            if (!col.blocks || !Array.isArray(col.blocks)) {
              throw new Error(`La columna #${cIdx + 1} de la página #${pIdx + 1} debe contener un array 'blocks'.`);
            }
