...
export default function AgendaColumn({ currentEvent, agendaEvents = [], eventCarouselIdx = 0, setEventCarouselIdx }) {
  ...
  return (
    <div className={`w-full flex-col flex border-[3px] ${borderColor} bg-white dark:bg-[#161722] text-black dark:text-white rounded-none ${shadowColor} transition-colors`}>
      ...
      {/* Header Cartelera */}
      <div className={`px-3 py-2 border-b-[3px] ${borderColor} bg-white dark:bg-[#1e1f2e] bg-[repeating-linear-gradient(-45deg,transparent,transparent_4px,rgba(31,41,55,0.15)_4px,rgba(31,41,55,0.15)_5px)] dark:bg-[repeating-linear-gradient(-45deg,transparent,transparent_4px,rgba(255,255,255,0.05)_4px,rgba(255,255,255,0.05)_5px)] flex items-center justify-between`}>
        <h3 className="font-black uppercase tracking-widest text-xl lg:text-2xl flex items-center gap-2 mt-1 truncate text-black dark:text-white" style={{ fontFamily: "'First Bunny', sans-serif" }}>
          <Ticket className="w-4 h-4 -mt-1 flex-shrink-0 text-red-500" /> <span className="truncate">Próximos Eventos</span>
        </h3>
        <button onClick={() => navigate('/EventosOffer')}
          className={`px-1.5 py-0.5 bg-white dark:bg-[#12131C] text-black dark:text-white border-[2px] ${borderColor} shadow-[2px_2px_0px_0px_rgba(31,41,55,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.1)] ${buttonHover} text-[8px] leading-none font-black uppercase transition-all flex-shrink-0`}
        >
          Ver Todo
        </button>
      </div>
      ...
      <div className="flex-1 bg-cream-bg dark:bg-[#0d0e15] relative overflow-hidden flex flex-col group transition-colors">
        {currentEvent ? (
          <>
            ...
            {/* Lista de Eventos */}
            <div className="flex flex-col flex-1 overflow-y-auto max-h-[350px] custom-scrollbar bg-cream-bg dark:bg-[#0d0e15]">
              {agendaEvents.map((evt, idx) => {
                const isSelected = idx === eventCarouselIdx;
                const hatchBg = "bg-white dark:bg-[#1e1f2e] bg-[repeating-linear-gradient(45deg,transparent,transparent_4px,rgba(31,41,55,0.1)_4px,rgba(31,41,55,0.1)_5px)] dark:bg-[repeating-linear-gradient(45deg,transparent,transparent_4px,rgba(255,255,255,0.05)_4px,rgba(255,255,255,0.05)_5px)]";
                return (
                  <div key={evt._id || idx} className={`group cursor-pointer p-4 border-b-[3px] ${borderColor} flex gap-4 transition-colors ${isSelected ? `${hatchBg} opacity-100` : `bg-transparent opacity-70 hover:opacity-100 hover:${hatchBg}`}`}
                    onClick={() => setEventCarouselIdx && setEventCarouselIdx(idx)}>

                    {/* Columna Izquierda: Título y Badges */}
                    <div className="flex flex-col flex-1 min-w-0 justify-between">
                      <h4 className={`text-xl font-bold uppercase leading-none tracking-tighter mb-4 transition-all text-black dark:text-white ${isSelected ? '[text-shadow:1px_1px_0px_#FF0000]' : 'group-hover:[text-shadow:1.5px_1.5px_0px_#FF0000]'}`} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                        {evt.nombreES || evt.nombre}
                      </h4>

                      <div className="flex flex-wrap gap-2 w-full mt-auto">
                        <div className={`flex items-center gap-1 border-[2px] px-2 py-1 shadow-[2px_2px_0px_0px_rgba(31,41,55,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.1)] transition-colors bg-white dark:bg-[#12131C] border-[#1F2937] dark:border-slate-600 text-black dark:text-white`}>
                          <Clock className={`w-3 h-3 transition-colors text-black dark:text-yellow-400 ${isSelected ? 'drop-shadow-[0.7px_0.7px_0px_#FF0000]' : 'group-hover:drop-shadow-[0.7px_0.7px_0px_#FF0000]'}`} />
                          <span className={`text-[9px] font-black uppercase tracking-widest transition-all text-black dark:text-white ${isSelected ? '[text-shadow:0.7px_0.7px_0px_#FF0000]' : 'group-hover:[text-shadow:0.7px_0.7px_0px_#FF0000]'}`}>{evt.horaInicio || 'TBD'}</span>
                        </div>
                        <div className={`flex items-center gap-1 border-[2px] px-2 py-1 shadow-[2px_2px_0px_0px_rgba(31,41,55,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.1)] transition-colors bg-white dark:bg-[#12131C] border-[#1F2937] dark:border-slate-600 text-black dark:text-white`}>
                    ...
                    {(evt.descripcionES || evt.descripcion) && (
                      <div className={`w-2/5 flex-shrink-0 border-l-[3px] ${borderColor} pl-4 flex items-center`}>
                        <p className={`text-[11px] lg:text-xs font-bold leading-tight line-clamp-4 transition-colors text-black dark:text-slate-300`} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                          {evt.descripcionES || evt.descripcion}
                        </p>
                      </div>
                    )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-black/50 dark:text-white/50 min-h-[300px]">
      ...
      {/* Botón Inferior */}
      <div className={`p-4 border-t-[3px] ${borderColor} bg-white dark:bg-[#161722] mt-auto`}>
        <button
          onClick={() => currentEvent && navigate(`/inscripcion/${currentEvent._id.substring(0, 8)}`)}
          disabled={!currentEvent}
          className={`w-full py-4 border-[3px] ${borderColor} bg-black dark:bg-yellow-400 text-white dark:text-black font-black uppercase tracking-[0.2em] shadow-[4px_4px_0px_0px_rgba(31,41,55,1)] dark:shadow-[4px_4px_0px_0px_rgba(250,204,21,0.5)] hover:bg-yellow-100 dark:hover:bg-white hover:text-black transition-all ${buttonHover} rounded-none disabled:opacity-50`}
        >
          Me Apunto
        </button>
      </div>
