...
// Eliminamos 'products' de los props, ya que usaremos 'allProveedores' de Redux
export function TableViewProveedores(/* { products = [] } */) {
  ...
  // Local overrides para mostrar los valores guardados en la UI (optimistic)
  ...
  const handleSaveRow = async (item) => {
    ...
    try {
      ...
      if (result) {
        // Si el dispatch devolvió el proveedor actualizado, usarlo en overrides.
        // Si no, usar los campos actualizados que enviamos.
        const newOverrideData = result && result._id ? result : { ...updatedFields, _id: item._id };
  ...
  const handleDelete = async (item) => {
    if (window.confirm(`¿Estás seguro de que deseas eliminar al proveedor ${item.Nombre_Proveedor}?`)) {
      try {
        ...
        // eliminar override local si existe
        setLocalOverrides(prev => {
          const copy = { ...prev };
  ...
  // Helper para obtener el valor que se debe mostrar: override -> item[field] -> fallback
  const getDisplayValue = (item, field, fallback = "") => {
    const overrideForItem = localOverrides[item._id] || {};
    const overrideVal = overrideForItem[field];
    const baseVal = item[field];
    const val = overrideVal !== undefined ? overrideVal : baseVal;
    if (val === null || val === undefined || (typeof val === "string" && String(val).trim() === "")) {
      return fallback;
    }
    return val;
  };

  // Mantener localOverrides consistente cuando cambian los proveedores desde Redux:
  useEffect(() => {
    const ids = new Set((allProveedores || []).map(p => p._id));
    setLocalOverrides(prev => {
      const next = {};
          ...
          key: 'nombre', content: (
            <td key="nombre" className="px-3 py-2 border-r border-gray-100 text-xs">
              {showEdit ?
                ...
                <span className="font-medium text-blue-800">{getDisplayValue(item, "Nombre_Proveedor", "Sin nombre")}</span>
              }
            </td>
          )
        },
          ...
          key: 'contactoNombre', content: (
            <td key="contactoNombre" className="px-3 py-2 border-r border-gray-100 text-xs">
              {showEdit ?
                ...
                <span className="text-gray-700">{getDisplayValue(item, "Contacto_Nombre", "N/A")}</span>
              }
            </td>
          )
        },
          ...
          key: 'contactoNumero', content: (
            <td key="contactoNumero" className="px-3 py-2 border-r border-gray-100 text-xs">
              {showEdit ?
                ...
                <span className="font-mono text-green-700">{getDisplayValue(item, "Contacto_Numero", "N/A")}</span>
              }
            </td>
          )
        },
          ...
          key: 'direccion', content: (
            <td key="direccion" className="px-3 py-2 border-r border-gray-100 text-xs">
              {showEdit ?
                ...
                <div className="text-gray-600 max-w-xs truncate" title={getDisplayValue(item, "Direccion", "")}>
                  {getDisplayValue(item, "Direccion", "Sin dirección")}
                </div>
              }
            </td>
          )
          ...
          key: 'nitcc', content: (
            <td key="nitcc" className="px-3 py-2 border-r border-gray-100 text-xs">
              {showEdit ?
                ...
                <span className="font-mono text-purple-700">{getDisplayValue(item, "NIT/CC", "N/A")}</span>
              }
            </td>
          )
        },
          ...
          key: 'paginaWeb', content: (
            <td key="paginaWeb" className="px-3 py-2 border-r border-gray-100 text-xs">
              {showEdit ?
                ...
                (() => {
                  const url = getDisplayValue(item, "PAGINA_WEB", "");
                  return url ? (
                    <a href={url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700 truncate max-w-xs block">
                      🔗 Ver sitio
                    </a>
                  ) : (
      ...
      <div className="bg-gray-50 p-4 border-b border-gray-200 mb-4 rounded-lg">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="text-sm text-gray-600">
            {/* Usamos 'allProveedores' para el conteo total */}
            Mostrando {sortedProducts.length} de {allProveedores.length} proveedores {/* <-- CAMBIO 3 */}
          </div>
        </div>
      </div>
