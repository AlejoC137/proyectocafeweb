...
export function TableViewManager({ products, currentType }) {
  ...
  // Función para manejar objetos anidados de forma segura (usada en Comanda)
  ...
  // Modificado para que los estados de Comanda sean "Terminado" y "Pendiente"
  const uniqueEstados = currentType === Comanda 
    ? ["Terminado", "Pendiente"]
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    ...
    // --- MODIFICACIÓN SORTING Comanda ---
    // Manejar ordenamiento de campos anidados de Comanda
    ...
    } else if (sortColumn === "Dates" && currentType !== Comanda) { // Evitar doble manejo
  ...
  const handleSaveRow = async (item) => {
    ...
    try {
      ...
      // --- MODIFICACIÓN GUARDADO Comanda ---
      else if (currentType === Comanda) {
        // Para Comanda, manejar objetos JSON anidados
  ...
  const renderActionButtons = (item, isEditing) => (
    <div className="flex gap-1">
      {/* Botón de Receta: visible para MenuItems y Procedimientos (o Comanda si tiene receta) */}
